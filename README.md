...
- **Contradiction detection is automatic, but scoped.** At ingest, when an updated **Entity or Concept**'s new facts/definition conflict with an already-linked peer, both are cross-linked via `wiki_contradictions` and left for review. Detection is bounded to **already-linked peers** — an Object that contradicts something it isn't linked to won't surface a finding yet. Note a current surfacing gap: **entity** contradictions are flagged by `wiki_lint` (severity `critical`), but **concept** contradictions are detected and cross-linked yet **not yet flagged by `wiki_lint`** — a planned follow-up. They are still recorded in `wiki_contradictions` and browsable in Anytype. Don't over-trust a clean contradiction column.
